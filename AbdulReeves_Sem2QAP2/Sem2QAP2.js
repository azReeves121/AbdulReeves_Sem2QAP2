...
function snake(str) {
  ...
  // Replacing spaces, tabs, and dots to underscores
...
function createVideo(url, width, controls) {
  const src = url.trim();
...
function parseDateString(dateString) {
  ...
  if (!regex.test(dateString)) {
    throw new Error("Error: Date must be in the format YYYY-MM-DD");
  }
  ...
  if (year < 1000 || year > 9999) {
    throw new Error("Error: Year must be a 4-digit number");
  }
  if (month < 1 || month > 12) {
    throw new Error("Error: Month must be a 2-digit number between 01 and 12");
  }
  if (day < 1 || day > 31) {
    throw new Error("Error: Day must be a 2-digit number between 01 and 31");
  }

  const date = new Date();
  date.setFullYear(year);
  date.setMonth(month - 1); // i found out months are 0-indexed in JavaScript Date objects...... it was a pain
  date.setDate(day);

  return date;
}

// error Testing lets play a gussing game

try {
  console.log(parseDateString("01-01-01")); // guess the error message
} catch (e) {
  console.error(e.message);
}

try {
  console.log(parseDateString("2021-1-01")); // guess the error message
} catch (e) {
  console.error(e.message);
}

try {
  console.log(parseDateString("2021-01-1")); // guess the error message
} catch (e) {
  console.error(e.message);
}

try {
  console.log(parseDateString(null)); // guess the error message
} catch (e) {
  console.error(e.message);
}

try {
  console.log(parseDateString(" I Love God ")); // guess the error message
} catch (e) {
  console.error(e.message);
}
/*******************************************************************************
...
function toDateString(date) {
  ...
  // Pading month and day with leading zeros
  if (month.length < 2) {
    month = "0" + month;
  }
  if (day.length < 2) {
...
// Test cases
try {
  console.log(toDateString(new Date(2021, 0, 1)));
  console.log(toDateString(new Date(2021, 8, 29)));
  console.log(toDateString(new Date(2021, 11, 31)));
} catch (e) {
  console.error(e.message);
}
...
// parseDateString and toDateString should be able to reverse each other
try {
  const dateStr = "2021-01-29";
  const parsedDate = parseDateString(dateStr);
  const convertedDateStr = toDateString(parsedDate);
  console.log(convertedDateStr === dateStr);
} catch (e) {
  console.error(e.message);
}
...
function normalizeCoord(string) {
  let lat, lng;
  ...
  // Triming whitespace and convert to numbers
...
// Error Testing
try {
  console.log(normalizeCoord("100, -200"));
} catch (e) {
  console.error(e.message);
}
...
// sorry i didnt want to type all the outputs
...
function generateLicenseLink(licenseCode, targetBlank = false) {
  ...
  let licenseText = licenseMap[licenseCode];
  let url;
...
console.log(generateLicenseLink("CC-UNKNOWN", true));
/*******************************************************************************
...
function pureBool(value) {
  ...
  if (typeof value === "boolean") {
    ...
    if (trueValues.has(lowerValue)) {
      return true;
    } else if (falseValues.has(lowerValue)) {
      return false;
    }
